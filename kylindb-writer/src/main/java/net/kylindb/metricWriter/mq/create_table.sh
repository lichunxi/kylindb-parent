#!/bin/sh
#how to run:  env COMPRESSION=NONE HBASE_HOME=path/to/hbase ./create_table.sh
...
create '$METRICS_TABLE',
  {NAME => 'm', VERSIONS => 1, COMPRESSION => '$COMPRESSION', BLOOMFILTER => '$BLOOMFILTER'}, {NUMREGIONS => $NUMREGIONS, SPLITALGO => 'UniformSplit'}

create '$NOTES_TABLE',
  {NAME => 'n', VERSIONS => 1, COMPRESSION => '$COMPRESSION', BLOOMFILTER => '$BLOOMFILTER'}, {NUMREGIONS => $NUMREGIONS, SPLITALGO => 'UniformSplit'}

create '$OFFSETS_TABLE',
  {NAME => 'o', VERSIONS => 1, COMPRESSION => '$COMPRESSION', BLOOMFILTER => '$BLOOMFILTER'}, {NUMREGIONS => $NUMREGIONS, SPLITALGO => 'UniformSplit'}
EOF
